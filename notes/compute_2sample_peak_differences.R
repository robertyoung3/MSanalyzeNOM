...
#'
#'
#' @param sample_1 a dataframe containing assigned formulas from get_sample_data()
#' @param sample_2 a dataframe containing assigned formulas from get_sample_data()
#' for another sample
#'
...
#'
#' @export
compute_2sample_peak_differences <- function (sample_1, sample_2){
  ...
  if (sum(!common_col_names %in% colnames(sample_1)) +
      sum(!common_col_names %in% colnames(sample_2)) > 0) {
    stop("Does not contain the required columns (produced by get_sample_data()).")
  }
