...
The Kroll diagram (Kroll et al. 2011) plots the average carbon oxidation state as a function of carbon number, where the bottom left reflects the smallest, most reduced compounds and the top right reflects the smallest, most oxidized compounds. As above, the  diagram can be colored by abundance to identify the region of the Kroll diagram where the greatest abundance lies.

```{r plot_Kroll_25_perc_groups, messages=FALSE, fig.height=3.5, fig.width=5}
plot_Kroll_25perc_groups(DOM_sample$new_assigned_formulas, plot_title = DOM_sample$sample_name)
```

As above, the *plot_Kroll_25perc_groups()* function can separate the assigned formulas by elemental class using the *panel* and *var_panel* arguments.
...
An R markdown notebook, such as this, or an R programming script should be saved with the "raw" FTICR MS data (e.g., the Excel file esported by PetroOrg&copy;) to facilitate reproducible analysis of the FTICR MS data. The named list created by the *get_sample_data()* function (i.e., "DOM_sample" in this example) can be stored as a separate R object to avoid reprocessing the "raw" FTICR MS data. 



## Future Work

Soon, the MSanalyzeNOM will contain additional functions for visualizing NOM data from FTICR MS analysis in other ways (e.g., HtoC vs. C number). If you have suggestions for other diagrams or features you'd like to see, please let me know. 
