...
In the example above, $C_{11}H_{16}O$ is not aromatic under any of the aromaticity measures. Nevertheless, when a search for $C_{11}H_{16}O$ is performed in [ChemSpider](http://www.chemspider.com/Search.aspx?q=C11H16O1), most of the resulting structures *are* aromatic, demonstrating that the aromaticity estimates are not perfect predictors in every case. In fact, FTICR MS does not distinguish between isomers unless chromatography or fragmentation are employed, so the chemical formula $C_{11}H_{16}O$ could represent a mixture of aromatic and non-aromatic structures. 

The sample's aromaticity can be estimated by summing the percent relative abundances of sample components with aromaticity estimates $\ge$ 0.5. Because NMR spectroscopy quantifies functional group content in whole samples, this estimate can be compared to NMR data. 
