...
If the highly abundant formulas are known  (e.g., linear alkylbenzene sulfonate surfactants), the van Krevelen diagram can be re-plotted without them, provided that the figure is appropriately annotated. After removing the LAS surfactants and stearic and palmitic acids referenced above, the van Krevelen diagram offers better resolution. 
...
This plot can be supplemented with data on the number of formulas in each category. In the example below, 1.5% of the formulas make up the first 25% of abundance, and 80.7% of the formulas make up the bottom 25%.
...
```{r data2 get_25perc_groups, message=FALSE}
...
```{r panel of element classes, warning=FALSE, messages=FALSE, fig.height=5, fig.width=8}
plot_VK_gradient_panel(DOM_sample$new_assigned_formulas, var = "rel_abund", plot_title = DOM_sample$sample_name, 
...
#### Kendrick Mass Defect and zscore
...
DOM_sample$assigned_formulas <- DOM_sample$assigned_formulas %>%
  compute_z_CH2() %>%
  compute_KMD_CH2(selection = "meas") %>%
...
DOM_sample$assigned_formulas %>%
  ...
  dplyr::select(chem_formula, mz, theor_mz, KMD_CH2_meas, KMD_CH2_theor) %>%
  head()
```
