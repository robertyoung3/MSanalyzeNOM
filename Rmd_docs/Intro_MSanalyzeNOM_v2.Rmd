...
```{r plot_VK_25perc_groups 25perc groups, messages=FALSE, warning=FALSE, fig.height=6, fig.width=7}
plot_VK_25perc_groups(DOM_sample$assigned_formulas, plot_title = DOM_sample$sample_name, panel = TRUE, var_panel = "class_element")
```


#### Kendrick Mass Defect and z score
...
The linear alkylbenzyl sulfonate (LAS) surfactants have a CH2 repeating unit, and a theoretical Kendrick mass defect equal to 0.1788, but the N4 O3 heteroatom class also contains a homologous series with a theoretical Kendrick mass defect equal to 0.1788. Z scores help to separate homologous series with similar measured and theoretical Kendrick mass defects.
...
The *remove_common_contaminants()* function removes LAS surfactants and stearic and palmitic acids from the assigned formulas to improve the visualization of less abundant NOM components. The function should only be used with appropriate annotations and discussion, and the edited formulas should be segregated from the unedited data in DOM_sample. Importantly, removing the contaminants also does not remove their influence on the other detected ions and assigned formulas, which can be reduced in number due to the presence of high abundance contaminants.
...
>**where:**  
>$OS_i$ = oxidation state of element $i$  
>$n_i$ = number of element $i$  
>$n_C$ = number of carbon atoms  

A simpler calculation for several comment elements is:

>$$4 - \frac{4c + h − 3n - 2o - 2s}{c}
>**where:**  
